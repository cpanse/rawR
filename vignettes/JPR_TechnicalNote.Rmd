...
Table: lists `rawrr` package functions connected to reading functionality. More details can be found in the package documentation (see supporting information, S-20 onward).
...
i <- c(9594, 11113, 11884, 12788, 12677, 13204, 13868, 14551, 16136, 17193,
  17612)
...
maxIonTime <- rawrr::makeAccessor(key = "Max. Ion Time (ms):",
    returnType = "double")
maxIonTime(S[[1]])
```
...
C <- rawrr::readChromatogram(rawfile, mass = iRTmz, tol = 10, type = "xic",
    filter = "ms")
class(C)
...
iRTscore <- c(-24.92, 19.79, 70.52, 87.23, 0, 28.71, 12.39, 33.38, 42.26,
    54.62, 100)
...
* Supp. Table 1 - Raw files available through MSV0000086542
* Supp. Table 2 - Overview of the reader functions in the `rawrr` package
* Supp. Table 3 - Instrument information extracted using `readFileHeader()`
* Supp. Table 4 - Highest-scoring MS2 scans for LGGNEQVTR++ across files listed in Supp. Table 1
* Supp. Figure 1 to 21 - Plots for highest-scoring scans listed in Supp. Table 4
* Supp. Figure 22 to 44 - iRT regression across files listed in Supp. Table 1
* Documentation for `rawrr` package, version 0.2.0
