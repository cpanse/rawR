...
Sparse vector normalization is done by dividing each vector component $x_{i}$ by the vector length $|x|$ (not to be confused with `length(x)`):

$$x' = \frac{x_{i}}{|x|}  =\frac{x_{i}}{\sqrt{\sum x_{i}^2}}$$
...
## Do the two ways give equal results?
...
$$ x \bullet y = |x||y| \cos \theta = \sum x_{i} \times y_{i}$$
...TODO : Show that `x' %*% y' is equal to `normDotProd(x, y)` or:

$$ x' \bullet y' = \frac{\sum x_{i} \times y_{i}}{|x||y|}$$ 
