...
Set the number of `mc.cores` to `r parallel::detectCores()`.
...
```{r fileHeader}
H <- parallel::mclapply(FUN=rawrr::readFileHeader, input)
```
...
.computeBestPeptideSpectrumMatch <- function(rawfile,
  ...
  bestMatchingMS2Scan <- parallel::mclapply(1:length(pepSeq), function(i){
...
The snippet below performs the peptide spectrum match number crunching for the `r names(iRT.mZ)[1]` peptide on all raw files utilizing `r parallel::detectCores()` cores:

```{r computeBestMatches}
start_time <- Sys.time()
bestMatchingMS2Scan <- parallel::mclapply(FUN = .computeBestPeptideSpectrumMatch,
                                input,
...
.plotBestMatches <- function(x, instrumentModel){
  ...
  S <- rawrr::readSpectrum(x$rawfile[1], x$scan)
  
  lapply(1, function(i){
    plot(S[[i]], main=x$peptide[i], sub=instrumentModel)
    Y <- x$peptide[i]
    abline(v=fi <- protViz::fragmentIon(Y)[[1]]$y[1:nchar(Y)-1],
           col="#AAAAAA66",lwd=5)
